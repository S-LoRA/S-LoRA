    ...
    for i in range(len(y_names)):
        ...
        for j, setting in enumerate(settings):
            ...
            for k in range(len(legends)):
                ...
                print(x, y)
                data[i][j].append((x, y))
    plot(figname, x_name, y_names, legends, settings, data)


def plot_req_rate(y_name):
    x_name = "Request Rate"
    legends = ["SLoRA", "SLoRA-bmm", "SLoRA-no-unify-mem"]
    settings = ["A10G (24GB) S2 (Llama-7b)", "A100 (80GB) S4 (Llama-13b)"]
    figname = f"Synthetic_workloads_about_the_{X_TAG[x_name]}_evaluate_{Y_TAG[y_name]}"
    ...
    for j, setting in enumerate(settings):
        ...
        for k in range(len(legends)):
            ...
            print(x, y)
            data[j].append((x, y))
    plot_single_row(figname, x_name, y_name, legends, settings, data)


if __name__ == "__main__":
    plot_num_adapter()
    plot_req_rate("Throughput (token/s)")
    plot_req_rate("First Token Latency (s)")
    plot_req_rate("SLO Attainment")
