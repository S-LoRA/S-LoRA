...
async def send_request(
    ...
    print(f"req_id {req_id} adapter_dir {adapter_dir} prompt_len {prompt_len} output_len {output_len} "
          f"request_latency {request_latency:.2f} s, first_token_latency {first_token_latency:.2f} s")
    return (adapter_dir, prompt_len, output_len, request_latency, first_token_latency)
...
def get_res_stats(per_req_latency, benchmark_time, backend, warmup_time=0, warmup_num=0):
    # get throughput
    num_abort = len([i for i in per_req_latency if i[4] is None])
    per_req_latency = [i for i in per_req_latency if i[4] is not None]
    ...
    avg_latency = np.mean([latency for _, _, _, latency, _ in per_req_latency])
    print(f"Average latency: {avg_latency:.2f} s")
    avg_per_token_latency = np.mean([
        latency / (prompt_len + output_len)
        for _, prompt_len, output_len, latency, _ in per_req_latency
    ])
    print(f"Average latency per token: {avg_per_token_latency:.2f} s")
    avg_per_output_token_latency = np.mean([
        latency / output_len
        for _, _, output_len, latency, _ in per_req_latency
    ])
    print("Average latency per output token: "
          f"{avg_per_output_token_latency:.2f} s")

    # compute the first token latency
    first_token_latency = [latency for _, _, _, _, latency in per_req_latency]
    ...
    attainment = [attainment_func(latency) for _, _, _, _, latency in per_req_latency]
    ...
    for adapter_dir, _, _, _, latency in per_req_latency:
        if adapter_dir not in ftl_per_adapter:
            ftl_per_adapter[adapter_dir] = [latency]
            attainment_per_adapter[adapter_dir] = [attainment_func(latency)]
        else:
            ftl_per_adapter[adapter_dir].append(latency)
            attainment_per_adapter[adapter_dir].append(attainment_func(latency))
